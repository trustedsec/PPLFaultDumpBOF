...
#define intZeroMemory(addr,size) MSVCRT$memset((addr),0,size)
    ...
    WINBASEAPI VOID WINAPI KERNEL32$AcquireSRWLockExclusive(PSRWLOCK SRWLock);
    WINBASEAPI WINBOOL WINAPI KERNEL32$SetFilePointerEx(HANDLE hFile, LARGE_INTEGER liDistanceToMove, PLARGE_INTEGER lpNewFilePointer, DWORD dwMoveMethod);
    WINBASEAPI HRESULT __stdcall
        CLDAPI$CfExecute(
             CONST CF_OPERATION_INFO* OpInfo,
              CF_OPERATION_PARAMETERS* OpParams
        );
    WINBASEAPI VOID WINAPI KERNEL32$ReleaseSRWLockExclusive(PSRWLOCK SRWLock);
    ...
    WINBASEAPI WINBOOL WINAPI KERNEL32$SetFileInformationByHandle(HANDLE hFile, FILE_INFO_BY_HANDLE_CLASS FileInformationClass, LPVOID lpFileInformation, DWORD dwBufferSize);
    ...
    WINBASEAPI
        HRESULT __stdcall
        CLDAPI$CfUnregisterSyncRoot(
             LPCWSTR SyncRootPath
        );
    WINBASEAPI WINBOOL WINAPI KERNEL32$GetFileAttributesExW(LPCWSTR lpFileName, GET_FILEEX_INFO_LEVELS fInfoLevelId, LPVOID lpFileInformation);
    ...
    LWSTDAPI_(LPWSTR) SHLWAPI$StrFormatByteSizeW(LONGLONG qdw, LPWSTR szBuf, UINT uiBufSize);
    WINBASEAPI HMODULE WINAPI KERNEL32$GetModuleHandleW(LPCWSTR lpModuleName);
    WINBASEAPI wchar_t* __cdecl MSVCRT$wcsncpy(wchar_t* _Dest, const wchar_t* _Source, size_t _Count);
    WINBASEAPI char* __cdecl MSVCRT$strncpy(char* _Dest, const char* _Source, size_t _Count);
    ...
    WINBASEAPI WINBOOL WINAPI CRYPT32$CryptEncodeObjectEx(DWORD dwCertEncodingType, LPCSTR lpszStructType, const void* pvStructInfo, DWORD dwFlags, PCRYPT_ENCODE_PARA pEncodePara, void* pvEncoded, DWORD* pcbEncoded);
    WINBASEAPI WINBOOL WINAPI CRYPT32$CryptBinaryToStringW(CONST BYTE* pbBinary, DWORD cbBinary, DWORD dwFlags, LPWSTR pszString, DWORD* pcchString);
